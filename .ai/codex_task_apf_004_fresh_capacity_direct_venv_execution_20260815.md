...
- 從任意初始 cwd 加入惡意 `PYTHONPATH` 與錯誤 `PYTHONHOME` 的 dry harness 已 PASS：executor 先切入 exact `<repo-root>`，Python `-E` 忽略 ambient env，載入 reviewed module path與 digest，且 `preflight_calls=0`。
...
3. exact payload 取自 `fresh_capacity_delivery_repair_plan_20260815/exact-next-command.json`，digest 必須為 `605400fbb270e9333fd9a595ee878116f54165a9b2d60dbdcb198694417cc717`。
4. Executor 必須先 resolve 並切入 exact reviewed `<repo-root>`，驗 cwd realpath、interpreter digest、reviewed module path與 digest；任一不符即在 preflight 前停止。
5. 不得呼叫 uv；直接使用既有 `<repo-root>/.venv/bin/python -E` public module入口，忽略 ambient `PYTHONPATH`／`PYTHONHOME`。
6. fresh capacity `preflight` 恰好呼叫一次；非零、ambiguous 或非 `PASS` 即停止，不得 retry。
7. 唯一 evidence root：
   `artifacts/fortune_council/content_writer_vnext_execution/apf_004_canary/publisher_fresh_capacity_direct_venv_20260815/`
8. 保存 pre/post snapshot、stdout/stderr、capacity payload、call trace、mutation summary、sanitizer與 digests；candidate commit，不 push。
