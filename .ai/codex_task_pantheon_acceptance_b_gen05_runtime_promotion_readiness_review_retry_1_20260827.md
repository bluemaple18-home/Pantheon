---
id: REVIEW-PANTHEON-ACCEPTANCE-B-GEN05-RUNTIME-PROMOTION-READINESS-20260827-RETRY-1
title: ↳ 審查｜驗證第五代執行環境升版就緒度
status: ready
...
supersedes: REVIEW-PANTHEON-ACCEPTANCE-B-GEN05-RUNTIME-PROMOTION-READINESS-20260827
---
...
- 只有可重現的 P0/P1 finding 可回 `REVIEW_NO_GO`；P2/P3 只記錄。

## 必查

1. 先作 task-semantic CodeGraph query，再由原始碼與實際 artifact bytes 確認；無結果才限域 `rg`。
2. 重算 candidate parent、changed files 與 allowlist；只允許單一 RESULT 與專屬 evidence directory。檢查本機絕對路徑、secret、dirty source 或未保存外部依賴。
3. 重跑既有 deterministic promotion plan validator，確認 target 是 `79884d8b...`、current actor/manifest/stage 與 plan digest/target manifest/generation 可重算，且 plan-only 不建 transaction root、不改 protected bytes。
4. 獨立驗證 Rule24 的兩個 cycles、host free、RSS、swap、reclamation、stop-loss、write-path/bytes/file budget；特別檢查 `planner-capacity-receipt-28f366-host.json` 的 freshness、execution identity、target binding 與權威性，舊 actor/manifest 或不同 execution line 的 PASS 不合格。
...
8. 核對 candidate 中 `28f366...` / `798...` 兩組 plan artifacts 的 supersession 單一明確，舊組不可被 downstream 當 authority。

## 驗證與交付

- 重跑 candidate 所列 JSON/schema/checksum/official gates 及 `git diff --check`；記錄 exact commands 與結果。
- 裁決只能是 `GEN05_RUNTIME_PROMOTION_READINESS_REVIEW_GO` 或 `REVIEW_NO_GO`。
- GO 列出 reviewed candidate/full parent、verified allowlist、Rule24、Rule25、promotion plan、continuation、zero-mutation 各自證據。
