...
const chrome = spawn(
  ...
  [
    ...
    `--user-data-dir=/tmp/pantheon-rwd-${Date.now()}`,
    "about:blank",
  ],
  { stdio: "ignore" },
);
...
try {
  ...
  const target = await fetch(`http://127.0.0.1:${port}/json/new?http://127.0.0.1:8000/personality`, {
    method: "PUT",
  ...
  await send(ws, "Page.navigate", { url: "http://127.0.0.1:8000/personality" });
  await wait(1000);
  const expression = `(() => {
  ...
  await writeFile(
    "TASK-002/evidence/personality-page-mobile-cdp-rwd.png",
