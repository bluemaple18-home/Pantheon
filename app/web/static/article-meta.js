...
function buildExpansionSections(article) {
  ...
  if (article.slug === "tarot-card-meanings") {
    return [
      {
        heading: "78 張塔羅牌要先看結構，不要一張一張死背",
        paragraphs: [
          "塔羅牌意思總覽最重要的不是把 78 張牌逐字背完，而是先看牌組結構。大阿爾克那多半描述人生階段、重大轉折和內在功課；小阿爾克那則更貼近日常，分別落在情緒、行動、資源和思考等面向。先分清這兩層，讀牌時會少很多混亂。",
          "小阿爾克那又可以從四個牌組理解：權杖偏行動與熱情，聖杯偏情緒與關係，寶劍偏想法與衝突，錢幣偏資源與現實條件。這些分類不是硬規則，但能幫你在感情、工作、財富和人生方向裡找到比較清楚的語氣。",
          "初學者常見的卡點，是看到牌就想立刻翻成結果。比較好的讀法，是先問：這張牌在提醒狀態、阻力、資源，還是下一步？同一張牌放在不同問題裡，答案會不一樣；牌義總覽提供語言，不替問題本身下結論。",
          "如果你想把牌義記得更穩，可以用情境分類，而不是只背關鍵字。先把每張牌放進感情、工作、人際或人生方向裡想一次，看看它提醒的是關係、行動、資源、想法，還是階段轉換。這樣記憶會比較有脈絡。",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "總覽文章適合當索引，不適合當最後判讀。真正讀牌時，還要回到牌陣中其他牌、問題問法、當事人的現實限制和可行選項。這些都看見後，牌義才會從抽象象徵變成能被討論的提醒。",
          "也可以把塔羅牌意思總覽當成查字典：先找到牌組，再看核心象徵，最後才看感情、工作或人生方向裡的語氣。不要從單一句關鍵字直接跳到結論，否則很容易把牌義讀得太窄，也忽略問題本身的條件。",
...
function buildNextStepSection(article, productTheme, managedArticle) {
  ...
  return {
    ...
    paragraphs: [
      ...
      "如果讀完後仍然覺得模糊，先不要急著換另一個答案。把目前的疑問改寫成一個更小的問題，例如「我想理解定義」、「我想釐清限制」或「我想知道下一步該讀哪一類文章」。問題變小，答案才會變清楚。",
    ],
  };
}
...
function buildReaderDecisionSection(article, productTheme) {
  ...
  if (article.product === "tarot") {
    return {
      heading: "讀完塔羅文章後，先把問題改寫得更清楚",
      paragraphs: [
        "讀完塔羅文章後，可以先把原本的問題改寫一次。把「會不會成功」改成「我現在缺哪個條件」，把「他會不會回來」改成「這段關係目前卡在哪裡」。問題越能描述狀態、阻力和下一步，牌義越不容易被讀成單一結果。",
        "塔羅文章適合拿來校準語言：這張牌比較像提醒我行動、等待、溝通，還是重新看待現實？它不能替任何人做承諾，也不能幫你控制對方。真正有用的是你讀完後更清楚自己能做什麼。",
        "如果你還是很想得到一個結果，先把結果題拆成可行動的問題。你可以問自己目前有哪些選項、哪個阻力最明顯、哪個行動最需要被確認。這樣塔羅文章才會成為整理工具，而不是焦慮放大器。",
        "最後再看牌義有沒有落到現實。若文章提醒你溝通，就要回到能不能開口、何時開口、要講哪件事；若文章提醒你等待，也要分清是需要準備，還是其實在逃避行動。",
      ],
    };
  }
  if (article.product === "fortune") {
    return {
      ...
      paragraphs: [
        ...
        "最後再問：這個觀察點能不能回到一個具體行動或判斷邊界？如果只能讓你更焦慮，就先停在背景知識；如果能幫你分清資料不足、時機未明或問題問錯，才適合繼續往下讀。",
      ],
    };
  }
  if (article.product === "astro") {
    return {
      ...
      paragraphs: [
        ...
        "最後把星盤語言翻成生活觀察。你可以問：這個落點在我平常的安全感、溝通方式或選擇節奏裡是否看得見？如果看不見，就先把它當成參考，不要硬套成完整答案。",
      ],
    };
  }
  return {
