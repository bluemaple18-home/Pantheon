...
function buildSearchIntentSection(article, productTheme) {
  ...
  return {
    ...
    paragraphs: [
      ...
      `比較穩的讀法，是先把通用意思說清楚，再把限制講出來。這樣你可以得到可引用的答案，也不會被推向沒有根據的個人結論。`,
    ],
  };
}
  ...
  if (article.product === "fortune") {
    return {
      ...
      paragraphs: [
        ...
        "最後再問：這個觀察點能不能回到一個具體行動或判斷邊界？如果只能讓你更焦慮，就先停在背景知識；如果能幫你分清資料不足、時機未明或問題問錯，才適合繼續往下讀。能對上現實條件，命盤語言才有用，也比較不會把壓力誤讀成答案。",
      ],
    };
  }
  if (article.product === "astro") {
