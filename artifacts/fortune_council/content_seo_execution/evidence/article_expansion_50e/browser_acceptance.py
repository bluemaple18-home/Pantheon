...
ARTICLE_PATHS = [
    "/articles/astrology/astrology-0065",
    "/articles/astrology/astrology-0070",
    "/articles/astrology/astrology-0071",
    "/articles/astrology/astrology-0082",
    "/articles/astrology/astrology-0083",
...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        evidence["latest"] = page.locator("[data-home-articles] a").evaluate_all("nodes => nodes.map(node => new URL(node.href).pathname)")
    ...
    evidence["checks"] = {
        ...
        "articlesValid": all(item["title"] and item["updated"] == "2026-07-19" and item["sectionCount"] == 4 and 4 <= item["faqCount"] <= 5 for item in evidence["articles"]),
        "mobileNoHorizontalOverflow": evidence["mobile"]["scrollWidth"] <= evidence["mobile"]["clientWidth"],
        "noTraceback": not evidence["tracebacks"],
        "noRelevantConsoleErrors": not [item for item in evidence["console"] if item["type"] == "error" and not item.get("location", {}).get("url", "").endswith("/favicon.ico")],
    ...
    (EVIDENCE_DIR / "browser-acceptance.json").write_text(json.dumps(evidence, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"go": evidence["go"], "checks": evidence["checks"], "articles": evidence["articles"]}, ensure_ascii=False))
