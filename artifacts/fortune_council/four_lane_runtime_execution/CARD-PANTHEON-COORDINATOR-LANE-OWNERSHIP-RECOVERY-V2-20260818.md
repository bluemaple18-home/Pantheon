---
id: CARD-PANTHEON-COORDINATOR-LANE-OWNERSHIP-RECOVERY-V2-20260818
status: ready
type: implementation
chain_id: PANTHEON-COORDINATOR-LANE-OWNERSHIP-RECOVERY-V2-20260818
role: implementation
cycle: 1
thickness: strict
risk: core-bounded
model: gpt-5.5
reasoning: high
model_reason: RCA 已固定且屬核心 state ownership 契約；使用 GPT-5.5 執行 bounded repair，避免全部使用 5.6。
...
evidence_path: .work/CARD-PANTHEON-COORDINATOR-LANE-OWNERSHIP-RECOVERY-V2-20260818/
---

# Coordinator lane ownership recovery V2

## 工作名稱 → 正在做什麼 → 現在狀態

Coordinator lane ownership recovery → 固化 lane/mode routing authority，避免缺 brief 拖垮整輪 → `READY / NEW CHAIN AUTHORIZED`

## 授權與邊界

- 使用者於 2026-08-18 明確同意建立全新正式 chain。
- 舊 chain 因 formal thread create 與 immutable reservation 問題停止；不得重送或操作其 reservation。
- 本卡是新的正式 implementation chain，但修復問題與程式範圍不變。
- 不啟動、不修改、不部署 production。

## Root question 與已證明 RCA

必讀：

- `artifacts/fortune_council/four_lane_runtime_execution/coordinator_publisher_causal_rca_20260818/rca.md`
- `artifacts/fortune_council/four_lane_runtime_execution/CARD-PANTHEON-COORDINATOR-LANE-OWNERSHIP-REPAIR-20260818.md`

已證明：`_migrate_pending_jobs` 對缺 `<run_dir>/brief.json` 的 active state 呼叫 `_lane_for_state`，拋出 `ValueError("active run brief is unavailable")`，使整個 Coordinator cycle 退出。Node scan 只是 amplifier；capacity guard 是正確後果；Publisher recovery 不屬本卡。

## 可改範圍

- `scripts/agy_gemini_coordinator.py`
- `tests/test_agy_gemini_coordinator.py`
- `.work/CARD-PANTHEON-COORDINATOR-LANE-OWNERSHIP-RECOVERY-V2-20260818/`
...
2. `_lane_for_state` 對新版本 state 只信 state；驗證 mode/lane pair，未知或衝突值 fail-closed，不得由可變 brief 覆寫。
