---
id: CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822-RETRY-1
...
cycle: 1
retry_of: CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822
supersedes_card: CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822
status: ready
...
ownership:
  - artifacts/fortune_council/four_lane_runtime_execution/CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822-RETRY-1-RESULT.md
  - artifacts/fortune_council/four_lane_runtime_execution/g8_current_production_readonly_reconciliation_v0370_20260822_retry_1/**
forbidden_scope:
...
verification:
  - provisioning HEAD、origin/main product authority 與 peeled v0.3.370 release authority 分層精確核對
...
evidence_path: artifacts/fortune_council/four_lane_runtime_execution/g8_current_production_readonly_reconciliation_v0370_20260822_retry_1/
result_path: artifacts/fortune_council/four_lane_runtime_execution/CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822-RETRY-1-RESULT.md
---
...
- Product main authority：`origin/main` 固定為換手 commit `9f48abaadc6474859f1e6f805ad36086b84e8700`，且必須包含 release baseline。
- Provisioning source authority：以 `prepare-create` receipt 的 `required_base_sha` 為準；worktree `HEAD` 必須精確等於該 SHA，該 SHA 必須包含本 Retry 卡且為 product main authority 的 descendant。card-only dispatch commits 不得冒充 product authority。
...
2. 唯讀驗 provisioning `HEAD` 精確等於 reservation source SHA、`origin/main` 精確等於 product authority、兩者 ancestry、annotated tag object與 peeled tag。
...
| gate | 必要判定 |
| --- | --- |
| Release authority | provisioning `HEAD`=reservation source；`origin/main`=product authority；兩者包含 baseline、peeled tag精確 |
...
1. `artifacts/fortune_council/four_lane_runtime_execution/g8_current_production_readonly_reconciliation_v0370_20260822_retry_1/` 下的 current raw／normalized evidence、before／after snapshots、gate matrix與 command receipt；
...
```text
你負責 CARD-PANTHEON-G8-CURRENT-PRODUCTION-READONLY-RECONCILIATION-V0370-20260822-RETRY-1，role=production-readonly-reconciliation-auditor。完整讀卡、handoff、State Contract、Edge Map、Cycle 29–34 RESULT與 Rule 24／25。先驗 clean tracked tree、provisioning HEAD／origin/main product authority／peeled v0.3.370，再確認 CodeGraph indexed HEAD並做 task-semantic query。只讀當下 actor／manifest／queue／state／transaction／live／stage／launchctl／receipts，執行既有 read-only reconciler與本卡 evidence內 synthetic readiness；before/after mutation tripwire必須證明保護面完全不變。只可新增本卡 Retry evidence目錄與唯一 RESULT；禁止 promotion、reset、Capacity preflight/install、activation、restage、canary、Publisher child、deploy、tag、push、schedule、steady autonomy，以及修改 source/tests/config/production。產出唯一 gate matrix與 GO／NO-GO／UNKNOWN；只有所有 current gates一致且 state=ST-CANARY-READY才可 GO，且 GO 後立刻停止等人工 bounded canary授權。
```
