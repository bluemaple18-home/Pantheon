...
- reset 前的 normal live、轉換後候選與 bootstrap 後 live 都走同一份 Publisher structural receipt；即使舊 cohort 沒有 Python identity 欄位，也無條件驗 barrier-exec 外層、child module、`max-runs=1` 與 exact-run-id 格式。
...
- GREEN：Publisher reset、live identity drift、child/outer argv drift、launchctl path drift、任一服務 PID、bootstrap rollback共 8 passed。
- Publisher-only bounded regression（含 reset）：20 passed，228 deselected。
