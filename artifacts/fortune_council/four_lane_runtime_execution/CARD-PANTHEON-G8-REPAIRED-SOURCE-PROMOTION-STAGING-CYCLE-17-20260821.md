...
- tooling Python：`/Users/mattkuo/Documents/Pantheon/.venv/bin/python`，必須先證明 Python 3.12.12；此 authority 只供 Gate A、plan/apply 與 promotion tooling，不能代驗另一份 manifest。
...
- `PANTHEON_PYTHON_PATH` 必須等於該 target manifest 的 canonical `python_executable`。預期值雖為 tooling venv，但只有 target manifest materialize 並完成 postcheck 後才取得 manifest-bound runtime authority。
...
PANTHEON_PYTHON_PATH=/Users/mattkuo/Documents/Pantheon/.venv/bin/python
...
- formal preflight authorization 必須是本次修正後全新、單次且綁定 exact argv digest、target actor SHA、manifest path/digest 與 manifest-bound Python 的 artifact；先前 public preflight count=`1` 已消耗，不得重用其 authorization 或計數。
...
2. 執行 zero-write deterministic promotion plan，鎖定 `target_manifest_digest`、exact apply argv 與 exact formal preflight argv；Gate A authorization 與 formal preflight authorization 必須分離。
...
- formal public preflight：全新 authorization、exact argv 含 `TMPDIR=/private/tmp`、authoritative target actor/manifest/digest/manifest-bound Python，invocation=`1`，exit `0` 且 transition `PASS/accepted`；若非 PASS，本卡交付 `BLOCKED / NO CANARY` 且 capacity install count=`0`。
