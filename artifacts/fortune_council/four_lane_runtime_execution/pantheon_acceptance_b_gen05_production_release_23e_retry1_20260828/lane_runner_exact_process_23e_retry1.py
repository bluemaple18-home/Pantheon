#!/usr/bin/env python3
"""Run one exact i18n-new lane runner job through the promoted barrier."""
...
QUEUE_ROOT = "/Users/mattkuo/Documents/Pantheon-canary-runtime-v8/queue/lanes/i18n-new"
...
def main() -> int:
    ...
    write_json(EVIDENCE_DIR / "lane-runner-writer-command-23e-retry1.json", command)
    completed = subprocess.run(command, cwd=str(ACTOR_ROOT), capture_output=True, text=True, check=False)
    (EVIDENCE_DIR / "lane-runner-writer-23e-retry1.stdout.txt").write_text(completed.stdout, encoding="utf-8")
    (EVIDENCE_DIR / "lane-runner-writer-23e-retry1.stderr.txt").write_text(completed.stderr, encoding="utf-8")
    (EVIDENCE_DIR / "lane-runner-writer-23e-retry1.returncode.txt").write_text(str(completed.returncode) + "\n", encoding="utf-8")
    if completed.stdout.strip().startswith("{"):
        try:
            write_json(EVIDENCE_DIR / "lane-runner-writer-23e-retry1.stdout.json", json.loads(completed.stdout))
