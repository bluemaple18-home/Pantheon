...
每個 work 的 `work_id` 是下列 canonical JSON 的 SHA-256 前 24 碼：`source_kind`、`article_id`、`locale`、`campaign_version`。`campaign_version` 必須是 trim 後非空、單行的值；同一輸入重跑結果不變，版本變更必定產生不同 identity。
...
| `rewrite` | `legacy_article_records` + `_existing_rewrite_inventory` | 同一 `campaign_version` 的既有 rewrite run 之 article id | `legacy` work item | 既有 `seed_legacy_rewrite_runs` / Publisher backlog |
| `i18n-new` | `new` publication candidate | 同一 `campaign_version` 的 translation run `(source_article_id, locale)` | 衍生 work item，無 translation run | 既有 multilingual enqueue |
| `i18n-rewrite` | `rewrite` publication candidate | 同一 `campaign_version` 的 translation run `(source_article_id, locale)` | 衍生 work item，無 translation run | 既有 multilingual enqueue |
