...
BASE_URL = (
    "http://127.0.0.1:5173/"
    "?studio=1&capture=1&prototype=mobius-patch-sphere"
)
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for view in ("front", "orbit", "side"):
            ...
            output = EVIDENCE / f"mobius-patch-sphere-{view}.png"
            page.screenshot(path=str(output), omit_background=True)
            captures[view] = str(output.relative_to(ROOT))

        motion_captures: dict[str, str] = {}
        for time in (0, 4, 8):
            page.evaluate(
                """(time) => {
                  window.__PANTHEON_STUDIO__.setView("front");
                  window.__PANTHEON_STUDIO__.setTime(time);
                }""",
                time,
            )
            output = EVIDENCE / f"mobius-patch-sphere-time-{time}.png"
            page.screenshot(path=str(output), omit_background=True)
            motion_captures[str(time)] = str(output.relative_to(ROOT))
        ...
        output_json = EVIDENCE / "mobius-patch-sphere-browser-evidence.json"
    ...
    assert len(result["runtime"]["pivots"]) == 5, result
    assert result["runtime"]["triangles"] < 180_000, result
