...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for mode, filename in [
            ...
            ("radius-heat", "debug-radius-heat.png"),
            ("curvature", "debug-curvature.png"),
            ("control-points", "debug-control-points.png"),
    ...
    forbidden = ("Inner", "Occlusion", "Aperture", "Ribbon", "Mobius", "Fade")
    ...
    for metric in metrics["curveMetrics"]:
        if metric["controlPointCount"] < 8 or metric["controlPointCount"] > 12:
            failures.append(f"{metric['id']}: control point count out of range")
        if metric["shellCoverage"] < 0.70:
            failures.append(
                f"{metric['id']}: shell coverage below 70%"
            )
        if metric["minRadius"] < 0.50:
            failures.append(
                f"{metric['id']}: radius below 0.50"
            )
        if metric["maxRadius"] > 1.02:
            failures.append(
                f"{metric['id']}: radius above 1.02"
            )
        if metric["selfIntersection"]:
            failures.append(f"{metric['id']}: approximate self-intersection")
        if metric["minBendingRadius"] < 0.18:
            failures.append(f"{metric['id']}: bending radius below 0.18")
    if metrics["centroidLength"] > 0.05:
        failures.append("curve centroid is outside 0.05")
    if metrics["extent"]["ratio"] > 1.10:
        failures.append("extent ratio is above 1.10")
    if metrics["nineGridDensity"][4] <= 0:
        failures.append("nine-grid center cell is empty")
    if metrics["nineGridMaxToMean"] > 2.0:
        failures.append("nine-grid density exceeds 2.0x mean")
    ...
    result = {
        ...
        "acceptance": {
            ...
            "extentRatioWithinTarget":
                runtime["metrics"]["extent"]["ratio"] <= 1.10,
            "centroidWithinTarget":
                runtime["metrics"]["centroidLength"] <= 0.05,
            "shellCoverageMinimumPass": all(
                metric["shellCoverage"] >= 0.70
                for metric in runtime["metrics"]["curveMetrics"]
            ),
            "radiusRangePass": all(
                metric["minRadius"] >= 0.50
                and metric["maxRadius"] <= 1.02
                for metric in runtime["metrics"]["curveMetrics"]
            ),
            "nineGridDensityPass":
                runtime["metrics"]["nineGridDensity"][4] > 0
                and runtime["metrics"]["nineGridMaxToMean"] <= 2.0,
            "failures": failures,
        },
    }
