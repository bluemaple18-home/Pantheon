...
BASE_URL = "http://127.0.0.1:5173/?studio=1&capture=1&prototype=two-mobius"
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for view in ("front", "orbit", "side"):
            ...
            output = EVIDENCE / f"two-mobius-{view}.png"
        ...
        runtime = page.evaluate(
            """() => {
              ...
              return {
                ...
                rotations: Object.values(runtime.bandPivots).map((pivot) => [
                  pivot.rotation.x,
                  pivot.rotation.y,
                  pivot.rotation.z,
                ]),
        ...
        output_json = EVIDENCE / "two-mobius-browser-evidence.json"
    ...
    expected_angle = 30 * 3.141592653589793 / 180
    second_rotation = result["runtime"]["rotations"][1]
    assert result["httpStatus"] == 200, result
    assert result["runtime"]["pivots"] == ["mobius-01", "mobius-02"], result
    assert result["runtime"]["rotations"][0] == [0, 0, 0], result
    assert all(abs(value - expected_angle) < 1e-9 for value in second_rotation), result
