...
def _create_run_adapter_single_exact_tuple(
    exact_tuples: Iterable[Mapping[str, str]],
) -> dict[str, str]:
    if isinstance(exact_tuples, (str, bytes)):
        raise ValueError("create-run adapter exact tuples must be a collection")
    ...
    if len(parsed) != 1:
        raise ValueError("single-source create-only adapter requires exactly one exact tuple")
    exact = parsed[0]
    if exact["lane"] not in APF_CREATE_RUN_ADAPTER_SOURCE_LANES:
        raise ValueError("single-source create-only adapter requires a source lane")
    return exact
...
def _create_single_source_run_adapter_plan(
    *,
    repo_root: Path,
    workset: Mapping[str, Any],
    exact_tuples: Iterable[Mapping[str, str]],
    ...
    if (
        ...
        != {"schema_version", "campaign_version", "lanes", "items", "summary"}
        or workset.get("schema_version") != 1
        or not isinstance(workset.get("lanes"), list)
        or not isinstance(workset.get("items"), list)
    ):
        raise ValueError("single-source create-only adapter workset is invalid")
    if workset.get("campaign_version") != campaign_version:
        raise ValueError("create-run adapter campaign version drift")
    items = workset["items"]
    if len(items) != 1:
        raise ValueError("single-source create-only adapter requires exactly one item")
    exact = _create_run_adapter_single_exact_tuple(exact_tuples)
    if workset.get("lanes") != [exact["lane"]]:
        raise ValueError("single-source create-only adapter workset lane drift")
    if _create_run_adapter_digest(workset) != workset_sha256:
        raise ValueError("create-run adapter workset SHA drift")
    item = _create_run_adapter_work_item(
        items[0],
    ...
    if item["lane"] not in APF_CREATE_RUN_ADAPTER_SOURCE_LANES:
        raise ValueError("single-source create-only adapter requires a source lane")
    if any(
        exact[key] != item[key]
        for key in ("lane", "work_id", "article_id", "locale")
    ...
    run_id = _create_run_adapter_source_run_id(item)
    mode = "create" if item["lane"] == "new" else "rewrite_existing_body"
    run_record = {
        "lane": item["lane"],
        "run_id": run_id,
        "mode": mode,
        ...
        "article_id": item["article_id"],
        "locale": item["locale"],
        "run_dir": str((resolved_run_root / run_id).resolve()),
        "stage": "source",
        "depends_on": [],
    }
    plan = {
        "schema_version": 1,
        "entrypoint": "scripts.agy_gemini_coordinator:create_single_source_run_adapter",
        "campaign_version": campaign_version,
        "workset_sha256": workset_sha256,
        "confirmed_payload_digest": confirmed_payload_digest,
        "activation_authorization_digest": activation_authorization_digest,
...
def _create_run_adapter_single_source_brief(
    plan: Mapping[str, Any],
    by_lane: Mapping[str, Mapping[str, str]],
) -> dict[str, dict[str, Any]]:
    repo_root = Path(str(plan["roots"]["repo_root"]))
    ...
    if lane == "new":
        return {
            lane: _create_run_adapter_new_brief(
                repo_root,
                by_lane[lane],
                str(record["run_id"]),
            )
    ...
    if lane == "rewrite":
        return {
            lane: _create_run_adapter_rewrite_brief(
                repo_root,
                by_lane[lane],
                str(record["run_id"]),
            )
...
def _translation_pending_payload(
    ...
    *,
    ...
    if materialized is not None:
        required = {
            "run_id", "source_run_id", "lane", "brief_sha256", "plan_digest",
            "pending_digest_before", "pending_digest_after", "registration_identity_digest",
        }
        if (
            ...
            or materialized.get("plan_digest") != pending["plan_digest"]
            or not isinstance(materialized.get("brief_sha256"), str)
            or SHA256_PATTERN.fullmatch(str(materialized["brief_sha256"])) is None
            or any(
                not isinstance(materialized.get(key), str)
                or SHA256_PATTERN.fullmatch(str(materialized[key])) is None
                for key in ("pending_digest_before", "pending_digest_after", "registration_identity_digest")
            )
        ):
            raise ValueError("translation materialized receipt binding is invalid")
        if materialized["pending_digest_before"] != _canonical_json_file_sha256(pending):
            raise ValueError("translation materialized receipt pre-terminal digest differs")
        after_basis = {
            **pending,
            "status": "materialized",
            "materialized": {
        ...
        if materialized["pending_digest_after"] != _create_run_adapter_digest(after_basis):
            raise ValueError("translation materialized receipt terminal digest differs")
...
def _translation_pending_source(
    ...
    return state, brief, candidate, source
...
def _translation_pending_selector_details(path: Path) -> tuple[Path, tuple[int, int]]:
    """驗證 exact receipt selector，並回傳不可替換的 inode identity。"""
    selected = Path(path)
    if not selected.is_absolute() or selected != selected.resolve(strict=False):
        raise ValueError("translation pending receipt selector is not canonical")
    if selected.is_symlink() or not selected.is_file():
        raise ValueError("translation pending receipt selector is unsafe")
    selected_stat = selected.stat()
    if selected_stat.st_uid != os.getuid() or selected_stat.st_mode & 0o022:
        raise ValueError("translation pending receipt selector is unsafe")
    parent = selected.parent
    if parent != parent.resolve(strict=True) or parent.is_symlink() or parent.stat().st_uid != os.getuid() or parent.stat().st_mode & 0o022:
        raise ValueError("translation pending receipt parent is unsafe")
    return selected, (selected_stat.st_dev, selected_stat.st_ino)


def _translation_pending_selector(path: Path) -> Path:
    """保留 selector 的薄 public validation seam。"""
    selected, _identity = _translation_pending_selector_details(path)
    return selected


def _translation_pending_revalidate_selector(
    path: Path,
    expected_identity: tuple[int, int],
) -> Path:
    """在 lock 中拒絕同路徑的 alias、symlink 或 inode replacement。"""
    selected, identity = _translation_pending_selector_details(path)
    if selected != path or identity != expected_identity:
        raise ValueError("translation pending receipt selector identity changed")
    return selected


def _translation_pending_registration_identity(
    queue_root: Path,
    pending: Mapping[str, Any],
    expected_brief: dict[str, Any],
) -> tuple[dict[str, Any], Path, dict[str, Any]]:
    state, run_dir = _preflight_translation_registration(queue_root, expected_brief)
    expected_envelope = multilingual.translation_identity_envelope(
        str(pending["source_article_id"]), str(pending["lane"])
    )
    envelope = _validate_identity_envelope(state.get("identity_envelope") if state is not None else None)
    if (
        state is None
        or run_dir != (queue_root / "translation-runs" / str(pending["run_id"])).resolve()
        or state.get("run_id") != pending["run_id"]
        or state.get("run_dir") != str(run_dir)
        or state.get("lane") != pending["lane"]
        or envelope != expected_envelope
    ):
        raise ValueError("translation registration identity differs")
    identity = {
        "run_id": pending["run_id"],
        "run_dir": str(run_dir),
        "routing_schema_version": envelope["schema_version"],
        "mode": envelope["mode"],
        "lane": envelope["lane"],
        "identity_envelope_digest": envelope["digest"],
        "brief_sha256": editorial_contracts.artifact_sha256(expected_brief),
    }
    return state, run_dir, {**identity, "digest": _create_run_adapter_digest(identity)}
...
def materialize_translation_pending_dependency(
    ...
    *,
    ...
    root = queue_root.resolve()
    selected, selector_identity = _translation_pending_selector_details(pending_receipt)
    ...
    pending, initial_materialized = _translation_pending_payload(
    ...
    pinned_pending_digest = (
        initial_materialized["pending_digest_before"]
        if initial_materialized is not None else initial_digest
    )
    if (
        expected_target_run_id != pending["run_id"]
        or expected_pending_digest != pinned_pending_digest
        or expected_plan_digest != pending["plan_digest"]
    ):
        raise ValueError("translation pending external pin differs")
    with _run_identity_lock(str(pending["run_id"]), root):
        selected = _translation_pending_revalidate_selector(selected, selector_identity)
        ...
        expected_brief: dict[str, Any] | None = None
        if materialized is not None:
            _source_state, _source_brief, _candidate, source = _translation_pending_source(
                root, pending, source_run_id
            )
            expected_brief = _campaign_translation_brief(
                {"run_id": source_run_id, "article_id": pending["source_article_id"]},
                source, str(pending["locale"]),
            )
            _state, _run_dir, registration_identity = _translation_pending_registration_identity(
                root, pending, expected_brief
            )
            if (
                materialized["brief_sha256"] != editorial_contracts.artifact_sha256(expected_brief)
                or materialized["registration_identity_digest"] != registration_identity["digest"]
                or materialized["pending_digest_before"] != pinned_pending_digest
            ):
            ...
            return {
                ...
                "registration_identity_digest": registration_identity["digest"],
        ...
        source_snapshot = _create_run_adapter_digest({
            "state": _source_state, "brief": _source_brief,
        ...
        if expected_brief["run_id"] != pending["run_id"]:
        ...
        _registration, _run_dir, registration_identity = _translation_pending_registration_identity(
            root, pending, expected_brief
        )
        after_source, after_brief, after_candidate, after_normalized = _translation_pending_source(
            root, pending, source_run_id
        )
        selected = _translation_pending_revalidate_selector(selected, selector_identity)
        ...
        if (
            ...
            or _create_run_adapter_digest({
                "state": after_source, "brief": after_brief,
                "candidate": after_candidate, "source": after_normalized,
            }) != source_snapshot
        ):
        ...
        proof = {
            ...
            "brief_sha256": editorial_contracts.artifact_sha256(expected_brief),
            "plan_digest": pending["plan_digest"],
            "pending_digest_before": initial_digest,
            "registration_identity_digest": registration_identity["digest"],
        }
        after_basis = {**pending, "status": "materialized", "materialized": proof}
        terminal = {**after_basis, "materialized": {**proof, "pending_digest_after": _create_run_adapter_digest(after_basis)}}
        selected = _translation_pending_revalidate_selector(selected, selector_identity)
    ...
    return {
        ...
        "plan_digest": pending["plan_digest"],
        "brief_sha256": terminal["materialized"]["brief_sha256"],
        "registration_identity_digest": terminal["materialized"]["registration_identity_digest"],
        "queue_mutation": True,
        "public_mutation": False,
    }
...
def create_single_source_run_adapter(
    *,
    repo_root: Path,
    workset: Mapping[str, Any],
    exact_tuples: Iterable[Mapping[str, str]],
    ...
    """將一筆已授權 source APF tuple 轉為 create-only run identity。"""
    plan, by_lane = _create_single_source_run_adapter_plan(
        repo_root=repo_root,
        workset=workset,
        exact_tuples=exact_tuples,
        run_root=run_root,
...
SAME_GENERATION_PLAN_RETRY_FILES = {
    "external_plan": "external-plan.json",
    "plan_operation": "plan-operation.json",
...
def retry_same_generation_locale_plan(
    run_dir: Path,
    queue_root: Path,
    *,
    expected_run_id: str,
    ...
    expected_archive_digest: str,
    expected_inbox_digest: str,
    expected_external_plan_digest: str,
    expected_plan_operation_digest: str,
    expected_planning_result_digest: str,
    expected_source_ref_map_digest: str,
    execute: bool = False,
) -> dict[str, Any]:
    """Quarantine a hash-bound failed same-generation locale plan cache."""
    if EXACT_RUN_ID_PATTERN.fullmatch(expected_run_id) is None or generation < 1 or JOB_ID_PATTERN.fullmatch(expected_job_id) is None:
        raise ValueError("same-generation locale plan retry identity is invalid")
    expected_digests = {"registry": expected_registry_digest, "attempt": expected_attempt_digest, "archive": expected_archive_digest, "inbox": expected_inbox_digest, "external_plan": expected_external_plan_digest, "plan_operation": expected_plan_operation_digest, "planning_result": expected_planning_result_digest, "source_ref_map": expected_source_ref_map_digest}
    if any(SHA256_PATTERN.fullmatch(digest) is None for digest in expected_digests.values()):
        raise ValueError("same-generation locale plan retry digest is invalid")
    root = queue_root.resolve()
    resolved_run_dir = run_dir.resolve(strict=True)
    gen_dir = resolved_run_dir / "generations" / f"{generation:02d}"
    quarantine = gen_dir / "planning-cache-quarantine" / expected_job_id
    receipt_path = quarantine / "retry-receipt.json"
    forbidden = ("candidate.json", "review.json", "article-operation.json", "review-operation.json", "external-review.json", "deterministic-findings.json")
    def validate_generation_boundary() -> None:
        if not gen_dir.is_dir() or (resolved_run_dir / "generations" / f"{generation + 1:02d}").exists():
            raise ValueError("same-generation locale plan retry generation boundary is invalid")
        if any((gen_dir / name).exists() for name in forbidden):
            raise ValueError("same-generation locale plan retry cannot cross writer/reviewer boundary")
    validate_generation_boundary()

    def validate_lane_job(registry: Mapping[str, Any]) -> tuple[str, Mapping[str, Path], dict[str, Any]]:
        ...
        if (
            attempt.get("job_id") != expected_job_id
            or attempt.get("attempt_status") != "succeeded"
            or archive.get("job_id") != expected_job_id
            or archive.get("role") != "writer"
            or inbox.get("job_id") != expected_job_id
            or archive.get("request_sha256") != inbox.get("request_sha256")
            or attempt.get("request_sha256") != archive.get("request_sha256")
        ):
            raise ValueError("same-generation locale plan retry job identity differs")
        return status, paths, {"archive": archive, "inbox": inbox, "attempt": attempt}

    def validate_generation_cache() -> tuple[str, Mapping[str, Path], dict[str, Any]]:
        source_paths = {key: gen_dir / name for key, name in SAME_GENERATION_PLAN_RETRY_FILES.items()}
        quarantine_paths = {key: quarantine / name for key, name in SAME_GENERATION_PLAN_RETRY_FILES.items()}
        source_present = [path.exists() for path in source_paths.values()]
        quarantine_present = [path.exists() for path in quarantine_paths.values()]
        if all(source_present) and not any(quarantine_present):
            paths, status = source_paths, "SOURCE"
        elif all(source_present) and all(quarantine_present) and receipt_path.is_file():
            paths, status = source_paths, "RESTORED"
        elif not any(source_present) and all(quarantine_present) and receipt_path.is_file():
            paths, status = quarantine_paths, "QUARANTINED"
        else:
            raise ValueError("same-generation locale plan retry cache shape differs")
        if any(_file_sha256(path) != expected_digests[key] for key, path in paths.items()):
            raise ValueError("same-generation locale plan retry generation artifact digest differs")
        if status == "QUARANTINED":
            existing = json.loads(receipt_path.read_text(encoding="utf-8"))
            if existing.get("action") != receipt["action"] or existing.get("run_id") != expected_run_id or existing.get("generation") != generation or existing.get("job_id") != expected_job_id or existing.get("digests") != expected_digests or existing.get("status") not in {"PREPARED", "QUARANTINED"}:
                raise ValueError("same-generation locale plan retry receipt identity differs")
        if status == "RESTORED":
            existing = json.loads(receipt_path.read_text(encoding="utf-8"))
            old_digests = existing.get("digests")
            if existing.get("action") != receipt["action"] or existing.get("run_id") != expected_run_id or existing.get("generation") != generation or existing.get("job_id") != expected_job_id or existing.get("status") not in {"PREPARED", "QUARANTINED"} or not isinstance(old_digests, dict) or any(_file_sha256(quarantine_paths[key]) != old_digests.get(key) for key in SAME_GENERATION_PLAN_RETRY_FILES):
                raise ValueError("same-generation locale plan retry receipt identity differs")
        plan_result = json.loads(paths["planning_result"].read_text(encoding="utf-8"))
        plan_operation = json.loads(paths["plan_operation"].read_text(encoding="utf-8"))
        if (
            plan_result.get("generation") != generation
            or plan_result.get("transport_status") != "EXTERNAL_PLAN_AVAILABLE"
            or plan_result.get("planning_contract_status") != "PLANNING_CONTRACT_FAILURE"
            or plan_operation.get("status") != "success"
            or plan_operation.get("role") != "writer"
        ):
            raise ValueError("same-generation locale plan retry planning artifact identity differs")
        return status, paths, plan_operation
    ...
    def validate_registry(registry: Mapping[str, Any], cache_status: str, job_status: str) -> str:
        ...
        if continuation.get("run_id") != expected_run_id or continuation.get("status") != "active" or continuation.get("next_generation") != generation:
            raise ValueError("same-generation locale plan retry continuation state differs")
        if registry.get("status") == "failed":
            if (
                job_status != ("QUARANTINED" if cache_status == "QUARANTINED" else "SOURCE")
                or _canonical_json_file_sha256(registry) != expected_registry_digest
                or registry.get("error_type") != "LocalePlanValidationError"
                or registry.get("last_job_id") not in {expected_job_id, None}
            ):
                raise ValueError("same-generation locale plan retry registry state differs")
            return "READY_TO_EXECUTE" if cache_status != "QUARANTINED" else "READY_TO_RESUME"
    ...
    receipt = {"action": "retry_same_generation_locale_plan", "run_id": expected_run_id, "generation": generation, "job_id": expected_job_id, "digests": expected_digests}
    context = _run_identity_lock(expected_run_id, root) if execute else nullcontext()
    with context:
        validate_generation_boundary()
        cache_status, cache_paths, plan_operation = validate_generation_cache()
        registry = read_run_state(resolved_run_dir, root)
        job_status, job_paths, lane_job = validate_lane_job(registry)
        identity_source = "registry_last_job_id"
        if registry.get("last_job_id") is None:
            identity_hashes = [payload.get(key) for payload in (plan_operation, lane_job["archive"]) for key in ("prompt_sha256", "schema_sha256")]
            if cache_status == "QUARANTINED" or job_status != "SOURCE" or any(not isinstance(value, str) or SHA256_PATTERN.fullmatch(value) is None for value in identity_hashes) or any(plan_operation.get(key) != lane_job["archive"].get(key) for key in ("prompt_sha256", "schema_sha256")):
                raise ValueError("same-generation locale plan retry job identity differs")
            identity_source = "plan_operation_lane_residue"
        status = validate_registry(registry, cache_status, job_status)
        receipt = {**receipt, "identity_source": identity_source, "selector": {key: registry.get(key) for key in ("lane", "mode", "routing_schema_version")}}
        if status == "ALREADY_ACTIVE" or not execute:
            return {**receipt, "execute": execute, "status": status}
        quarantine.mkdir(parents=True, exist_ok=True)
        if cache_status != "QUARANTINED":
        ...
        return {**receipt, "execute": execute, "status": "RETRY_READY", "after_digest": _canonical_json_file_sha256(read_run_state(resolved_run_dir, root))}
...
def parse_args() -> argparse.Namespace:
    ...
    retry_plan = subparsers.add_parser("retry-same-generation-locale-plan")
    retry_plan.add_argument("run_dir", type=Path)
    retry_plan.add_argument("--run-id", required=True)
    retry_plan.add_argument("--generation", type=int, required=True)
    retry_plan.add_argument("--expected-job-id", required=True)
    for name in ("registry", "attempt", "archive", "inbox", "external-plan", "plan-operation", "planning-result", "source-ref-map"):
        retry_plan.add_argument(f"--expected-{name}-digest", required=True)
    retry_plan.add_argument("--execute", action="store_true")
    ...
    materialize_pending.add_argument("--expected-target-run-id", required=True)
    materialize_pending.add_argument("--expected-pending-digest", required=True)
    materialize_pending.add_argument("--expected-plan-digest", required=True)
...
def main() -> int:
    ...
    if args.command == "register":
    ...
    elif args.command == "retry-same-generation-locale-plan":
        try:
            digest_args = {f"expected_{key}_digest": getattr(args, f"expected_{key}_digest") for key in ("registry", "attempt", "archive", "inbox", "external_plan", "plan_operation", "planning_result", "source_ref_map")}
            result = retry_same_generation_locale_plan(args.run_dir, queue_root, expected_run_id=args.run_id, generation=args.generation, expected_job_id=args.expected_job_id, execute=args.execute, **digest_args)
        except (OSError, UnicodeError, json.JSONDecodeError, ValueError) as error:
        ...
        try:
            result = materialize_translation_pending_dependency(
                args.repo_root.resolve(), queue_root, source_run_id=args.source_run_id[0],
                pending_receipt=args.pending_receipt[0],
                expected_target_run_id=args.expected_target_run_id,
                expected_pending_digest=args.expected_pending_digest,
