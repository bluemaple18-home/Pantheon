...
class AcceptanceSealedReplayBundle:
    ...
    activation_token_digest: str = ""

    def with_activation_token_digest(
        self,
        activation_token_digest: str,
    ) -> "AcceptanceSealedReplayBundle":
        if SAFE_SHA256.fullmatch(activation_token_digest) is None:
            raise ValueError("sealed replay bundle activation token digest is invalid")
        return replace(self, activation_token_digest=activation_token_digest)
...
def _validate_acceptance_sealed_bundle_formal_runtime(
    queue_root: Path,
    lane: str,
) -> str:
    if os.environ.get("PANTHEON_FORMAL_RUNTIME") != "1":
        raise ValueError("sealed replay bundle formal runtime is required")
    if lane not in CONTENT_LANES:
        raise ValueError("sealed replay bundle lane is invalid")
    service_label = f"com.pantheon.agy-gemini-{lane}"
    receipt = formal_runtime.validate_runtime_tick(
        service_label,
        queue_root=queue_root.resolve(),
        state_root=Path(
            os.environ.get("PANTHEON_RUNTIME_PUBLISHER_STATE_ROOT", Path.cwd())
        ),
        actor_root=Path(os.environ.get("PANTHEON_RUNTIME_ACTOR_ROOT", Path.cwd())),
        log_root=Path(os.environ.get("PANTHEON_RUNTIME_LOG_ROOT", Path.cwd())),
        require_activation_token=True,
    )
    activation_token_digest = receipt.get("activation_token_digest")
    if (
        type(activation_token_digest) is not str
        or SAFE_SHA256.fullmatch(activation_token_digest) is None
    ):
        raise ValueError("sealed replay bundle activation token digest is invalid")
    return activation_token_digest
...
def _load_exact_pending_bundle_request(
    ...
    _assert_bundle_entry_claimable_from_pending_outbox(queue_root, bundle, matches[0])
...
def _assert_bundle_entry_claimable_from_pending_outbox(
    ...
    state = _classify_bundle_entry_delivery(queue_root, bundle, entry)
...
def _sealed_bundle_entry_binding(entry: AcceptanceSealedReplayEntry) -> dict[str, object]:
    return {
...
def _sealed_bundle_delivery_authority_digest(
    bundle: AcceptanceSealedReplayBundle,
    entry: AcceptanceSealedReplayEntry,
) -> str:
    session_authority_digest = _sealed_bundle_session_authority_digest(bundle)
    authority = {
        "authority_schema": "pantheon.acceptance.sealed_bundle.entry.v1",
        "entry": _sealed_bundle_entry_binding(entry),
        "session_authority_digest": session_authority_digest,
    }
    return hashlib.sha256(_canonical_json_bytes(authority)).hexdigest()


def _sealed_bundle_session_authority_digest(
    bundle: AcceptanceSealedReplayBundle,
) -> str:
    if SAFE_SHA256.fullmatch(bundle.activation_token_digest) is None:
        raise ValueError("sealed replay bundle activation token digest is not bound")
    authority = {
        "activation_token_digest": bundle.activation_token_digest,
        "authority_schema": "pantheon.acceptance.sealed_bundle.session.v1",
...
def _sealed_bundle_attempt_id(
    bundle: AcceptanceSealedReplayBundle,
    entry: AcceptanceSealedReplayEntry,
) -> str:
    suffix = _sealed_bundle_delivery_authority_digest(bundle, entry)[:32]
    return f"sr2-{suffix}"
...
def _classify_bundle_entry_delivery(
    ...
    anchor_path = (
        ...
        / "v4"
        / "anchors"
        / f"{entry.job_id}.{_sealed_bundle_attempt_id(bundle, entry)}.json"
    )
    ...
    broker_result = run_single_shot(
        ...
        attempt_id=_sealed_bundle_attempt_id(bundle, entry),
        request_sha256=entry.request_sha256,
        model=entry.model,
    ...
    expected_receipt = ExecutionReceipt(
        entry.job_id,
        entry.namespace,
        _sealed_bundle_attempt_id(bundle, entry),
        entry.request_sha256,
        entry.model,
        RAW_STDIN_PROFILE,
...
def _trusted_bundle_usage_count(
    ...
    *,
    ...
    for entry in bundle.entries:
        try:
            entry_used = _trusted_bundle_entry_used(queue_root, bundle, entry)
        except ValueError:
            if pending_entry is not None and entry.entry_id == pending_entry.entry_id:
                _assert_bundle_entry_claimable_from_pending_outbox(queue_root, bundle, entry)
                entry_used = False
            else:
                raise
        if entry_used:
...
def sealed_replay_bundle_process_once(
    *,
    ...
    queue_root = queue_root.resolve()
    activation_token_digest = _validate_acceptance_sealed_bundle_formal_runtime(
        queue_root,
        lane,
    )
    actor_root = Path(os.environ.get("PANTHEON_RUNTIME_ACTOR_ROOT", Path.cwd())).resolve()
    ...
    bundle = bundle.with_activation_token_digest(activation_token_digest)
    session_authority_digest = _sealed_bundle_session_authority_digest(bundle)
    _assert_acceptance_sealed_replay_environment()
    ...
    def sealed_transport(
        ...
        refreshed_token_digest = _validate_acceptance_sealed_bundle_formal_runtime(
            queue_root,
            lane,
        )
        if refreshed_token_digest != activation_token_digest:
            raise ValueError("sealed replay bundle activation token digest drift")
        if _sealed_bundle_session_authority_digest(bundle) != session_authority_digest:
            raise ValueError("sealed replay bundle session authority drift")
    ...
    def validate_claimed_request(claimed_request: dict[str, Any]) -> None:
        refreshed_token_digest = _validate_acceptance_sealed_bundle_formal_runtime(
            queue_root,
            lane,
        )
        ...
        if _sealed_bundle_session_authority_digest(bundle) != session_authority_digest:
            raise ValueError("sealed replay bundle session authority drift")
        entry.validate_request(claimed_request)
    ...
    result = _process_once(
        ...
        exact_run_ids=[exact_run_id],
        acceptance_sealed_replay=True,
        claimed_request_validator=validate_claimed_request,
    )
    if result.get("status") == "processed":
        result["sealed_replay_bundle"] = {
            ...
            "expected_bundle_digest": bundle.expected_bundle_digest,
            "session_authority_digest": session_authority_digest,
            "activation_token_digest": activation_token_digest,
...
def _bundle_namespace_job_ids(
    ...
    known_attempts = {
        f"{entry.job_id}.{_sealed_bundle_attempt_id(bundle, entry)}"
        for entry in bundle.entries
    }
...
def sealed_replay_bundle_close(
    *,
    ...
    entry_states = {
        entry.entry_id: _classify_bundle_entry_delivery(queue_root, bundle, entry)
        for entry in bundle.entries
    }
    incomplete = [
        entry.entry_id
