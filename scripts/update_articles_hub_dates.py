from __future__ import annotations

import argparse
import re
import sys
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPO_ROOT))


def articles_hub_updated_date(markup: str) -> str:
    match = re.search(r'<time datetime="([^"]+)" data-articles-updated>', markup)
    if match is None:
        raise RuntimeError("articles hub updated date marker is missing")
    return match.group(1)


def render_articles_hub_dates(
    markup: str,
    *,
    published_date: str | None = None,
    updated_date: str | None = None,
) -> str:
    if published_date is None and updated_date is None:
        from main import ARTICLE_PUBLISHED_DATE, ARTICLES_HUB_UPDATED_DATE

        published_date = ARTICLE_PUBLISHED_DATE
        updated_date = ARTICLES_HUB_UPDATED_DATE

    replacements: list[tuple[str, str]] = []
    if published_date is not None:
        replacements.extend(
            (
                (
                    r'(<meta property="article:published_time" content=")[^"]+(" />)',
                    rf"\g<1>{published_date}\g<2>",
                ),
                (
                    r'("datePublished": ")[^"]+(")',
                    rf"\g<1>{published_date}\g<2>",
                ),
                (
                    r'(<time datetime=")[^"]+(" data-articles-published>)[^<]+(</time>)',
                    rf"\g<1>{published_date}\g<2>{published_date}\g<3>",
                ),
            )
        )
    if updated_date is not None:
        replacements.extend(
            (
                (
                    r'(<meta property="article:modified_time" content=")[^"]+(" />)',
                    rf"\g<1>{updated_date}\g<2>",
                ),
                (
                    r'("dateModified": ")[^"]+(")',
                    rf"\g<1>{updated_date}\g<2>",
                ),
                (
                    r'(<time datetime=")[^"]+(" data-articles-updated>)[^<]+(</time>)',
                    rf"\g<1>{updated_date}\g<2>{updated_date}\g<3>",
                ),
            )
        )
    for pattern, replacement in replacements:
        markup, count = re.subn(pattern, replacement, markup, count=1)
        if count != 1:
            raise RuntimeError(f"找不到唯一日期欄位：{pattern}")
    return markup


def main() -> int:
    from main import ARTICLE_PUBLISHED_DATE, ARTICLES_HUB_UPDATED_DATE, WEB_DIR

    parser = argparse.ArgumentParser(description="同步最新文章頁的發布與更新日期。")
    parser.add_argument("--check", action="store_true", help="只檢查靜態頁是否與日期參數同步。")
    parser.add_argument("--updated-date", help="指定最新文章頁的最後更新日期（YYYY-MM-DD）。")
    args = parser.parse_args()

    articles_html = WEB_DIR / "articles.html"
    current = articles_html.read_text(encoding="utf-8")
    rendered = render_articles_hub_dates(
        current,
        published_date=ARTICLE_PUBLISHED_DATE,
        updated_date=args.updated_date or ARTICLES_HUB_UPDATED_DATE,
    )
    if args.check:
        return 0 if rendered == current else 1
    articles_html.write_text(rendered, encoding="utf-8")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
