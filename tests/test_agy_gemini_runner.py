...
def _write_counting_sealed_executable(
    tmp_path: Path,
    counter: Path,
    result: dict[str, object] | None = None,
) -> Path:
    payload = json.dumps(result or {"ok": True}, ensure_ascii=False, sort_keys=True)
    executable = tmp_path / "sealed-provider-counting.py"
    executable.write_text(
        "#!/usr/bin/env python3\n"
        "from pathlib import Path\n"
        "import sys\n"
...
def _rewrite_bundle_with_same_semantics(bundle_path: Path, target_name: str) -> Path:
    bundle = json.loads(bundle_path.read_text(encoding="utf-8"))
    target = bundle_path.with_name(target_name)
    target.write_text(
...
def _install_formal_bundle_cli_environment(
    queue: Path,
    bundle_path: Path,
    *,
    lane: str,
    ...
    bundle = json.loads(bundle_path.read_text(encoding="utf-8"))
    formal_root = bundle_path.resolve().parent / "formal-runtime"
    manifest_queue = formal_root / "queue"
    state = formal_root / "state"
    logs = formal_root / "logs"
    ready = formal_root / "ready"
    lane_alias = manifest_queue / "lanes" / lane
    for path in (manifest_queue / "lanes", state, logs, ready):
        path.mkdir(parents=True, exist_ok=True)
    queue.parent.mkdir(parents=True, exist_ok=True)
    if not lane_alias.exists() and not lane_alias.is_symlink():
        lane_alias.symlink_to(queue)
    generation = os.environ.get("PANTHEON_RUNTIME_GENERATION") or str(
        bundle.get("generation", "generation-r2")
    )
    manifest = runtime_manifest.build_manifest(
        actor_root=Path.cwd().resolve(),
        queue_root=manifest_queue.resolve(),
        publisher_state_root=state.resolve(),
        log_root=logs.resolve(),
        identity=f"sealed-bundle-{lane}",
        runtime_digest=hashlib.sha256(str(bundle_path).encode("utf-8")).hexdigest(),
        config_version="runtime-v2",
        generation=generation,
        uv_executable=Path(sys.executable).resolve(strict=True),
    )
    manifest_path = formal_root / "manifest.json"
    barrier = formal_root / "activation.barrier"
    ...
    os.environ.update(
        {
            ...
            "PANTHEON_RUNTIME_SERVICE_LABEL": f"com.pantheon.agy-gemini-{lane}",
...
def _run_sealed_bundle_cli(
    ...
    *,
    ...
    install_formal_runtime: bool = True,
    ...
    try:
        if install_formal_runtime:
            _install_formal_bundle_cli_environment(queue, bundle_path, lane=lane)
        sys.argv = [
...
def _tree_snapshot(root: Path) -> dict[str, bytes]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            snapshot[str(path.relative_to(root))] = os.readlink(path).encode("utf-8")
        elif path.is_file():
...
def test_sealed_bundle_cli_processes_writer_then_reviewer_with_one_bundle(
    ...
    before_restart_close = _queue_snapshot(queue)
    restart_close_exit, restart_close_payload = _run_sealed_bundle_close_cli(
        capsys,
        queue,
        bundle_path,
    )
    ...
    assert restart_close_payload["sealed_replay_bundle_session"]["delivered_entries"] == [
        "writer",
        "reviewer",
    ]
    assert _queue_snapshot(queue) == before_restart_close
...
def test_sealed_bundle_requires_formal_runtime_before_pending_scan(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scripts import agy_gemini_runner as runner
    ...
    with pytest.raises(ValueError, match="formal runtime is required"):
        runner.sealed_replay_bundle_process_once(
            queue_root=queue,
            lane="new",
            exact_run_id="target-run",
            bundle_path=bundle_path,
            expected_bundle_digest=hashlib.sha256(bundle_path.read_bytes()).hexdigest(),
        )
...
def test_sealed_bundle_close_rejects_generation_rebound_delivery_evidence(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("AGY_GEMINI_V4_BROKER", raising=False)
    monkeypatch.setenv("PANTHEON_RUNTIME_GENERATION", "generation-a")
    queue = tmp_path / "queue"
    request = create_external_request(
        queue,
        namespace=_namespace_for_run_id("target-run"),
    ...
    bundle_a = _write_sealed_bundle(
        tmp_path,
        queue,
        [_bundle_entry(request, executable)],
        generation="generation-a",
    )
    assert _run_sealed_bundle_cli(capsys, queue, bundle_a)[0] == 0
    bundle_b = _rewrite_bundle_body(
        bundle_a,
        "sealed-bundle-generation-b.json",
        {"generation": "generation-b"},
    )
    monkeypatch.setenv("PANTHEON_RUNTIME_GENERATION", "generation-b")
...
def test_sealed_bundle_close_rejects_same_semantics_different_raw_digest_reuse(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    executable = _write_sealed_executable(tmp_path)
    compact_bundle = _write_sealed_bundle(tmp_path, queue, [_bundle_entry(request, executable)])
    assert _run_sealed_bundle_cli(capsys, queue, compact_bundle)[0] == 0
    pretty_bundle = _rewrite_bundle_with_same_semantics(
        compact_bundle,
        "sealed-bundle-pretty.json",
    )
    assert hashlib.sha256(compact_bundle.read_bytes()).hexdigest() != hashlib.sha256(
        pretty_bundle.read_bytes()
    ).hexdigest()

    exit_code, payload = _run_sealed_bundle_close_cli(capsys, queue, pretty_bundle)
...
@pytest.mark.parametrize(
    ("case", "bundle_overrides"),
    [
        ("provider_budget", {"provider_call_budget": 1}),
        ("accepted_base", {"accepted_base_sha": ACCEPTED_BASE_PARENT_SHA}),
    ],
)
def test_sealed_bundle_close_rejects_bundle_body_rebound_delivery_evidence(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
    case: str,
...
def test_sealed_bundle_claim_time_runtime_token_drift_restores_outbox(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    bundle_path = _write_sealed_bundle(tmp_path, queue, [_bundle_entry(request, executable)])
    calls = 0
    original_validate_runtime_tick = runner.formal_runtime.validate_runtime_tick

    def drift_after_claim(*args: object, **kwargs: object) -> dict[str, object]:
        nonlocal calls
        calls += 1
        receipt = dict(original_validate_runtime_tick(*args, **kwargs))
        if calls >= 3:
            receipt["activation_token_digest"] = "f" * 64
        return receipt
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "processing",
        "failed",
        "rebound_session_authority",
        "wrong_activation_token",
    ],
)
def test_sealed_bundle_close_classifier_is_zero_write_for_delivery_matrix(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
    case: str,
    ...
    try:
        ...
        executable = _write_counting_sealed_executable(tmp_path, counter)
        bundle_path = _write_sealed_bundle(
            tmp_path,
            queue,
            [_bundle_entry(request, executable)],
        ...
        close_bundle = bundle_path
        expected_exit = 0 if case == "delivered" else 64
        expected_status = "closed" if case == "delivered" else "rejected"
        ledger = queue / "v4" / "ledger" / f"{job_id}.jsonl"
        anchors = list((queue / "v4" / "anchors").glob(f"{job_id}.*.json"))
        assert len(anchors) == 1
        anchor = anchors[0]
        if case == "missing_inbox":
            (queue / "inbox" / f"{job_id}.json").unlink()
        elif case == "missing_archive":
            (queue / "archive" / f"{job_id}.json").unlink()
        elif case == "missing_ledger":
            ledger.unlink()
        elif case == "missing_anchor":
            anchor.unlink()
        elif case == "corrupt_ledger":
            ledger.write_text("not-jsonl\n", encoding="utf-8")
        elif case == "anchor_mismatch":
            anchor.rename(anchor.with_name(f"{job_id}.sr2-mismatch.json"))
        elif case == "delivered_outbox":
            (queue / "outbox" / f"{job_id}.json").write_bytes(
                (queue / "archive" / f"{job_id}.json").read_bytes()
            )
        elif case == "processing":
            (queue / "processing").mkdir(exist_ok=True)
            (queue / "processing" / f"{job_id}.json").write_bytes(
                (queue / "archive" / f"{job_id}.json").read_bytes()
            )
        elif case == "failed":
            (queue / "failed").mkdir(exist_ok=True)
            (queue / "failed" / f"{job_id}.json").write_bytes(
                (queue / "archive" / f"{job_id}.json").read_bytes()
            )
        elif case == "rebound_session_authority":
            close_bundle = _rewrite_bundle_with_same_semantics(
                bundle_path,
                "sealed-bundle-rebound-raw.json",
            )
        _install_formal_bundle_cli_environment(queue, close_bundle, lane="new")
        if case == "wrong_activation_token":
            token = Path(os.environ["PANTHEON_RUNTIME_ACTIVATION_TOKEN"])
            payload = json.loads(token.read_text(encoding="utf-8"))
            payload["generation"] = "generation-wrong-token"
            token.write_text(json.dumps(payload, sort_keys=True), encoding="utf-8")
        before_tree = _tree_snapshot(tmp_path)
        before_counter = counter.read_text(encoding="utf-8")

        exit_code, payload = _run_sealed_bundle_close_cli(
            capsys,
            queue,
            close_bundle,
            install_formal_runtime=False,
        )

        assert exit_code == expected_exit
        assert payload["status"] == expected_status
        assert _tree_snapshot(tmp_path) == before_tree
        assert counter.read_text(encoding="utf-8") == before_counter
    finally:
        os.environ.clear()
        os.environ.update(old_env)
