...
NATIVE_QUALITY_PROMPT_FIXTURE = json.loads(
    (
        Path(__file__).parent
        / "fixtures/agy_multilingual_translation_quality/native_quality_prompt_cases.json"
...
def test_saved_japanese_plan_accepts_source_derived_hyphenated_proper_name() -> None:
    brief = translation_brief("ja")
    source = brief["articles"][0]["source"]
    source["bodySections"][0]["paragraphs"][0] += (
        " Rider–Waite–Smith 版の構図も確認します。"
    )
    brief["articles"][0]["source_sha256"] = multilingual.source_sha256(source)
    external = external_locale_plan(brief)
    external["articles"][0]["source_sha256"] = brief["articles"][0]["source_sha256"]
    external["articles"][0]["coverage_mapping"][0]["coverage_note"] = (
        "Rider-Waite-Smith版の構図について"
    )
...
@pytest.mark.parametrize(
    "case",
    NATIVE_QUALITY_PROMPT_FIXTURE["cases"],
    ids=lambda case: case["locale"],
)
def test_native_quality_repair_contract_is_bound_to_locale_prompts(
    ...
    findings = [
        ...
        for finding in case["reviewer_findings"]
    ]
    plan_prompt = multilingual._plan_prompt(
        brief,
        generation=2,
    ...
    article_prompt = multilingual._article_prompt(brief, plan, findings)

    assert case["plan_marker"] in plan_prompt
    assert case["article_marker"] in article_prompt
    assert "SOURCE_SYNTAX_TRANSFER" in plan_prompt
    assert "NON_NATIVE_SEARCH_INTENT" in article_prompt
...
def test_native_quality_fixture_preserves_reviewer_hard_reject(
    case: dict[str, object],
) -> None:
    locale = str(case["locale"])
    brief = translation_brief(locale)
    candidate = translation_candidate(locale)
    review = multilingual._review_generated_candidate(
        brief,
        candidate,
        {
            "articles": [
                {
                    "slot": "article-01",
                    "verdict": "REJECT",
                    "findings": case["reviewer_findings"],
                }
            ]
        },
        [],
    ...
    assert not multilingual._review_approved(review)
    assert {
        finding["code"]
        for article in review["articles"]
        for finding in article["findings"]
    } == {"NON_NATIVE_SEARCH_INTENT", "SOURCE_SYNTAX_TRANSFER"}


def test_english_prompt_control_is_byte_for_byte_stable() -> None:
    brief = non_tarot_translation_brief("en")
    external = external_locale_plan(brief)
    plan = multilingual._hydrate_locale_plan(
        brief,
        external,
    ...
    article_prompt = multilingual._article_prompt(brief, plan, [])
    control = NATIVE_QUALITY_PROMPT_FIXTURE["english_control"]

    assert hashlib.sha256(plan_prompt.encode("utf-8")).hexdigest() == control[
        "plan_prompt_sha256"
    ]
    assert hashlib.sha256(article_prompt.encode("utf-8")).hexdigest() == control[
